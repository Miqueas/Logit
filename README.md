...
Logger provides 6 functions/methods:
  ...
  Arguments (optional):
    - (__string__) `name` The name of your application/project/script/etc. Basically a name that you can identify with something in particular, is useful if you plan to use several instances of Logger on a single proyect.
    ...
    - (__any__) `...` Varargs used with `string.format()` for the `header` argument. (`string.format()`)
  ...
  - `log(msg [, lvl, ...])`: The main function that writes logs. In a Logger instance it can be called with `log()` instead of `log:log()`. Arguments:
    ...
    - (__any [opcional]__) `...` Varargs used with `string.format()` to place values in `msg`. (`string.format()`)
